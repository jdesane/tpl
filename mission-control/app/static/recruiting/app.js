...
// ════════════════════════════════════════
function checkAuth() {
  authenticated = localStorage.getItem('recruiting_auth') === 'true';
  if (authenticated) {
    showApp();
  } else {
...
function handleLogin() {
  ...
  if (AUTH_USERS[email] && AUTH_USERS[email] === password) {
    authenticated = true;
    localStorage.setItem('recruiting_auth', 'true');
...
function logout() {
  localStorage.removeItem('recruiting_auth');
  authenticated = false;
  $('loginScreen').style.display = 'flex';
  $('appShell').classList.remove('visible');
}
